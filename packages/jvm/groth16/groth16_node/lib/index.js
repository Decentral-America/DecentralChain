...
function randrange(from, to) {
  ...
  if (typeof from === 'number') return from + Math.floor(Math.random() * interval);
  let t = 0;
  while (interval > 1n << BigInt(t * 8)) t++;
...
function utxoAccumulator(mpc_params, data) {
  ...
  const roots = [zero_hash, pair_hash].map((leaf, i) =>
    MerkleTree.getRoot(data.proof_sibling[i], data.proof_index, leaf, 1),
  );
  const publicInputs = [...data.note_hashes, data.proof_index, ...roots];
  return { proof, publicInputs };
}

const nullifier = (hash, sk) =>
  debufferizeBigints(native.nullifier(bufferizeBigints(hash), bufferizeBigints(sk)));
...
class MerkleTree {
  constructor(proof_length) {
    ...
    this._merkleState = Array(proof_length + 1)
      .fill(0)
      .map(() => []);
  }
  ...
  pushMany(elements) {
    ...
    for (let i = 1; i <= this.proof_length; i++) {
      ...
      for (let j = index >> BigInt(i); j <= (index + s) >> BigInt(i); j++) {
        this._merkleState[i][j] = merkleHash(
          this.cell(i - 1, j * 2n),
          this.cell(i - 1, j * 2n + 1n),
          i - 1,
        );
      }
    }
  ...
  proof(index, offset) {
    ...
    return Array(this.proof_length)
      .fill(0)
      .map((o, i) => this.cell(i, (index >> BigInt(i)) ^ 1n))
      .slice(offset);
  }
  ...
  static getRoot(proof, index, leaf, offset) {
    ...
    for (let i = 0; i < proof.length; i++) {
      root =
        ((index >> BigInt(i + offset)) & 1n) == 0n
          ? merkleHash(root, proof[i], i + offset)
          : merkleHash(proof[i], root, i + offset);
    }
    return root;
  }
}
