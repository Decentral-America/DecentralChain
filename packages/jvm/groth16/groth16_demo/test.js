...
const {
  ...
  debufferizeBigints,
  note_hash,
} = require('../groth16_node/lib/index.js');
...
const transfer_mpc = fs.readFileSync('../groth16_setup/mpc_params_transfer');
const accumulator_mpc = fs.readFileSync('../groth16_setup/mpc_params_accumulator');
...
let out_note = [
  utxo(0n, 0n, deposit_amount - accumulatorFee, fr_random(), pk),
  utxo(0n, 0n, 0n, fr_random(), pk),
];
...
(async () => {
  console.log(
    `Waves balance before transfer:\t\t${await nodeInteraction.balance(address(seed, chainId), rpc)}`,
  );
  ...
  let tx = invokeScript(
    {
      ...
      call: {
        ...
        args: [
          { type: 'binary', value: res.proof.toString('base64') },
          {
            type: 'binary',
            value: Buffer.concat(bufferizeBigints(res.publicInputs)).toString('base64'),
          },
  ...
  await sleep(10000);
  console.log(
    `Waves balance after transfer:\t\t${await nodeInteraction.balance(address(seed, chainId), rpc)}`,
  );
