...
import static io.decentralchain.transactions.serializers.json.JsonSerializer.JSON_MAPPER;
...
import io.decentralchain.sdk.json.DccMapper;
import io.decentralchain.sdk.json.TypeRef;
import io.decentralchain.sdk.peers.BlacklistedPeer;
...
public class Node implements Closeable {
  ...
  public Node(String url, HttpClient httpClient)
      throws URISyntaxException, IOException, NodeException {
  ...
  public long getBalance(Address address) throws IOException, NodeException {
    return asJson(get("/addresses/balance/" + address.toString())).get("balance").asLong();
  }
  ...
  public List<DataEntry> getData(Address address, List<String> keys)
    ...
    return asType(
        post("/addresses/data/" + address.toString())
            .setEntity(JSON_MAPPER.writeValueAsString(jsonBody), "application/json"),
        TypeRef.DATA_ENTRIES);
  }
  ...
  */
  public List<DataEntry> getData(Address address, Pattern regex) throws IOException, NodeException {
    return asType(
        get("/addresses/data/" + address.toString()).addParameter("matches", regex.toString()),
        TypeRef.DATA_ENTRIES);
  }
  ...
  public DataEntry getData(Address address, String key) throws IOException, NodeException {
    ...
    return asType(
        get("/addresses/data/" + address.toString() + "/" + encodedKey), TypeRef.DATA_ENTRY);
  }

  @Deprecated
  public long getEffectiveBalance(Address address) throws IOException, NodeException {
    return asJson(get("/addresses/effectiveBalance/" + address.toString())).get("balance").asLong();
  }
  ...
  public ScriptMeta getScriptMeta(Address address) throws IOException, NodeException {
    ...
    if (json.hasNonNull("meta")) return mapper.convertValue(json.get("meta"), TypeRef.SCRIPT_META);
    else return new ScriptMeta(0, new HashMap<>());
  }
  ...
  public AssetDistribution getAssetDistribution(
      AssetId assetId, int height, int limit, Address after) throws IOException, NodeException {
    NodeRequest request =
        get("/assets/" + assetId.toString() + "/distribution/" + height + "/limit/" + limit);
    if (after != null) request.addParameter("after", after.toString());
  ...
  public List<AssetBalance> getAssetsBalance(Address address) throws IOException, NodeException {
    return mapper
        .readerFor(TypeRef.ASSET_BALANCES)
        .readValue(asJson(get("/assets/balance/" + address.toString())).get("balances"));
  }

  public List<AssetBalance> getAssetsBalance(Address address, List<AssetId> assetIds)
      throws IOException, NodeException {
    ObjectNode jsonBody = JSON_MAPPER.createObjectNode();
  ...
  public AssetDetails getAssetDetails(AssetId assetId) throws IOException, NodeException {
    return asType(
        get("/assets/details/" + assetId.toString()).addParameter("full", "true"),
  ...
  public int getBlockHeight(Base58String blockId) throws IOException, NodeException {
    return asJson(get("/blocks/height/" + blockId.toString())).get("height").asInt();
  }

  public int getBlockHeight(long timestamp) throws IOException, NodeException {
    return asJson(get("/blocks/heightByTimestamp/" + timestamp)).get("height").asInt();
  }

  public long getBlocksDelay(Base58String startBlockId, int blocksNum)
      throws IOException, NodeException {
  ...
  public List<BlockHeaders> getBlocksHeaders(int fromHeight, int toHeight)
      throws IOException, NodeException {
    return asType(
        get("/blocks/headers/seq/" + fromHeight + "/" + toHeight), TypeRef.BLOCKS_HEADERS);
  }
  ...
  public List<Block> getBlocksGeneratedBy(Address generator, int fromHeight, int toHeight)
      throws IOException, NodeException {
    return asType(
        get("/blocks/address/" + generator.toString() + "/" + fromHeight + "/" + toHeight),
        TypeRef.BLOCKS);
  }
   ...
   */
  public List<CommittedGenerator> getCommittedGeneratorsAt(int height)
      throws IOException, NodeException {
   ...
   */
  public int getCommittedGeneratorIndex(Address address, int height)
      throws IOException, NodeException {
  ...
  public List<Peer> getAllPeers() throws NodeException, IOException {
    return mapper.readerFor(TypeRef.ALL_PEERS).readValue(asJson(get("/peers/all")).get("peers"));
  }
  ...
  public List<ConnectedPeer> getConnectedPeers() throws NodeException, IOException {
    return mapper
        .readerFor(TypeRef.CONNECTED_PEERS)
  ...
  public <T extends Transaction> Validation validateTransaction(T transaction)
      throws IOException, NodeException {
    return asType(
        post("/debug/validate").setEntity(transaction.toJson(), "application/json"),
        TypeRef.VALIDATION);
  }
  ...
  public <T extends Transaction> Amount calculateTransactionFee(T transaction)
    ...
    JsonNode json =
        asJson(
            post("/transactions/calculateFee").setEntity(transaction.toJson(), "application/json"));
    return Amount.of(
        json.get("feeAmount").asLong(), JsonSerializer.assetIdFromJson(json.get("feeAssetId")));
  }
  ...
  public <T extends Transaction> T broadcast(T transaction) throws IOException, NodeException {
    ...
    return (T)
        asType(
            post("/transactions/broadcast").setEntity(transaction.toJson(), "application/json"),
            TypeRef.TRANSACTION);
  }

  public EthRpcResponse broadcastEthTransaction(EthereumTransaction ethTransaction)
      throws IOException, NodeException {
  ...
  /**
   * Returns info about requested transactions.
   *
   * @param txIds IDs of transactions
   * @return info about requested transactions
   * @throws IOException if a network error occurs
   * @throws NodeException if the node returns an error
   */
  public List<TransactionInfo> getTransactionsInfo(List<Id> txIds)
      throws IOException, NodeException {
  ...
  /**
   * Returns typed info about requested transactions.
   *
   * <p>Use this method ONLY if you're sure all requested transactions are of the same type.
   *
   * @param txIds IDs of transactions
   * @param transactionInfoClass info class for all the requested transactions
   * @return typed info about requested transactions
   * @throws IOException if a network error occurs
   * @throws NodeException if the node returns an error
   */
  public <T extends TransactionInfo> List<T> getTransactionsInfo(
      List<Id> txIds, Class<T> transactionInfoClass) throws IOException, NodeException {
    return getTransactionsInfo(txIds).stream()
  ...
  public List<TransactionInfo> getTransactionsByAddress(Address address, int limit, Id afterTxId)
    ...
    if (afterTxId != null) request.addParameter("after", afterTxId.toString());
    ...
    return mapper.readerFor(TypeRef.TRANSACTIONS_INFO).readValue(inner);
  }

  public TransactionStatus getTransactionStatus(Id txId) throws IOException, NodeException {
    return asType(
            get("/transactions/status").addParameter("id", txId.toString()),
            TypeRef.TRANSACTIONS_STATUS)
        .get(0);
  }
  ...
  /**
   * For local compilation use {@link io.decentralchain.sdk.util.CompilationUtil#compile(String,
   * boolean, boolean)}.
   */
  ...
  public TransactionInfo waitForTransaction(Id id, int waitingInSeconds) throws IOException {
    ...
    if (waitingInSeconds < 1)
      throw new IllegalStateException(
          "waitForTransaction: waiting value must be positive. Current: " + waitingInSeconds);

    Exception lastException = null;
    for (long spentMillis = 0;
        spentMillis < waitingInSeconds * 1000L;
        spentMillis += pollingIntervalInMillis) {
      try {
    ...
    throw new IOException(
        "Could not wait for transaction " + id + " in " + waitingInSeconds + " seconds",
        lastException);
  }
  ...
  public <TI extends TransactionInfo, T extends Transaction> TI waitForTransaction(
      T tx, Class<TI> infoClass) throws IOException {
  ...
  public void waitForTransactions(List<Id> ids, int waitingInSeconds)
    ...
    for (long spentMillis = 0;
      ...
      try {
        ...
        if (statuses.stream().allMatch(s -> CONFIRMED.equals(s.status()))) return;
      } catch (Exception e) {
  ...
  public int waitForHeight(int target, int waitingInSeconds) throws IOException, NodeException {
    ...
    if (waitingInSeconds < 1)
      throw new IllegalStateException(
          "waitForHeight: value must be positive. Current: " + waitingInSeconds);
    ...
    throw new IllegalStateException(
        "Could not wait for the height to rise from "
            + start
            + " to "
            + target
            + ": height "
  ...
  public int waitBlocks(int blocksCount, int waitingInSeconds) throws IOException, NodeException {
    if (waitingInSeconds < 1)
      throw new IllegalStateException(
          "waitBlocks: waiting value must be positive. Current: " + waitingInSeconds);
  ...
  /**
   * Lightweight internal HTTP request builder. Accumulates path, query parameters, headers, and an
   * optional body, then converts to a {@link HttpRequest} at execution time. This type is an
  ...
  private final class NodeRequest {
    ...
    NodeRequest addParameter(String name, String value) {
      ...
      queryParams
          .append(URLEncoder.encode(name, StandardCharsets.UTF_8).replace("+", "%20"))
          .append('=')
  ...
  protected <T> T asType(NodeRequest request, TypeReference<T> reference)
      throws IOException, NodeException {
  ...
  protected NodeRequest buildSendRawTransactionRq(String rawData) throws JsonProcessingException {
    return post("/eth")
        .setEntity(
            new EthRpcRequest(
                    "2.0", "eth_sendRawTransaction", Collections.singletonList(rawData), 0)
                .toJsonString(),
            "application/json");
  }
  ...
  protected EthRpcResponse handleEthResponse(ObjectNode rs)
      throws NodeException, JsonProcessingException {
    // Standard JSON-RPC 2.0: errors are at the top level, not inside result.
    if (rs.hasNonNull("error")) {
      JsonNode error = rs.get("error");
      throw new NodeException(
          error.path("code").asInt(-1), error.path("message").asText("Ethereum RPC error"));
    }
