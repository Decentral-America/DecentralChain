...
import io.decentralchain.sdk.info.InvokeScriptTransactionInfo;
...
import io.decentralchain.transactions.invocation.IntegerArg;
...
public class LeasingTest extends BaseTestWithNodeInDocker {
  ...
  void leaseInfo() throws IOException, NodeException {
    ...
    Base64String dAppScript =
        node.compileScript(
                "{-# STDLIB_VERSION 5 #-}\n{-# CONTENT_TYPE DAPP #-}\n{-# SCRIPT_TYPE ACCOUNT #-}\n"
                    + "@Callable(inv)\nfunc lease(amount: Int) = [Lease(inv.caller, amount)]\n"
                    + "@Callable(inv)\nfunc cancel(leaseId: String) = [LeaseCancel(leaseId.fromBase58String())]")
            .script();
    node.waitForTransaction(
        node.broadcast(SetScriptTransaction.builder(dAppScript).getSignedWith(bob)).id());
    ...
    TransactionInfo leaseTx =
        node.waitForTransaction(
            node.broadcast(
                    LeaseTransaction.builder(bob.address(), leaseAmount).getSignedWith(alice))
                .id());

    TransactionInfo invokeTx =
        node.waitForTransaction(
            node.broadcast(
                    InvokeScriptTransaction.builder(
                            bob.address(), Function.as("lease", IntegerArg.as(invokeLeaseAmount)))
                        .getSignedWith(alice))
                .id());
    LeaseInfo stateChangesLease =
        node.getTransactionInfo(invokeTx.tx().id(), InvokeScriptTransactionInfo.class)
            .stateChanges()
            .leases()
            .get(0);
    ...
    assertThat(invokeLeasing)
        .isEqualTo(
            new LeaseInfo(
                stateChangesLease.id(),
                invokeTx.tx().id(),
                bob.address(),
                alice.address(),
                invokeLeaseAmount,
    ...
    TransactionInfo leaseCancelTx =
        node.waitForTransaction(
            node.broadcast(LeaseCancelTransaction.builder(leasing.id()).getSignedWith(alice)).id());

    TransactionInfo invokeCancelTx =
        node.waitForTransaction(
            node.broadcast(
                    InvokeScriptTransaction.builder(
                            bob.address(),
                            Function.as("cancel", StringArg.as(invokeLeasing.id().toString())))
                        .getSignedWith(alice))
                .id());

    LeaseInfo stateChangesCancel =
        node.getTransactionInfo(invokeCancelTx.tx().id(), InvokeScriptTransactionInfo.class)
            .stateChanges()
            .leaseCancels()
            .get(0);
    ...
    List<LeaseInfo> leasingListCancel =
        node.getLeasesInfo(leaseTx.tx().id(), stateChangesCancel.id());
    List<LeaseInfo> activeLeasesCancel = node.getActiveLeases(alice.address());

    // assert canceled leasing

    assertThat(leasingCancel)
        .isEqualTo(
            new LeaseInfo(
                leaseTx.tx().id(),
                leaseTx.tx().id(),
    ...
    // NOTE: Node 1.3.5 bug workaround (DCC-era) — retained for compatibility
