...
import io.decentralchain.sdk.BlockHeaders;
...
public class BlocksTest extends BaseTestWithNodeInDocker {
  ...
  void lastBlock() throws IOException, NodeException {
    ...
    int txHeight =
        node.waitForTransaction(
                node.broadcast(
                        DataTransaction.builder(StringEntry.as("foo", "bar")).getSignedWith(alice))
                    .id())
            .height();
    ...
    assumeTrue(
        block.id().equals(headers.id()), "Failed to request the same block with two requests");
  ...
  void transactions() throws IOException, NodeException {
    ...
    TransactionInfo transfer =
        node.waitForTransaction(
            node.broadcast(TransferTransaction.builder(bob, Amount.of(10)).getSignedWith(alice))
                .id());
    TransactionInfo leasing =
        node.waitForTransaction(
            node.broadcast(LeaseTransaction.builder(bob, 20).getSignedWith(alice)).id());

    TransactionWithStatus transferInBlock =
        node.getBlock(transfer.height()).transactions().stream()
            .filter(i -> transfer.tx().id().equals(i.tx().id()))
            .findFirst()
            .orElseThrow(
                () -> new IOException("Can't find transfer tx at height " + transfer.height()));
    TransactionWithStatus leasingInBlock =
        node.getBlocks(leasing.height(), leasing.height() + 1).get(0).transactions().stream()
            .filter(i -> leasing.tx().id().equals(i.tx().id()))
            .findFirst()
            .orElseThrow(
                () -> new IOException("Can't find leasing tx at height " + leasing.height()));
    ...
    assertThat(transferInBlock)
        .isEqualTo(new TransactionWithStatus(transfer.tx(), ApplicationStatus.SUCCEEDED));
    assertThat(leasingInBlock)
        .isEqualTo(new TransactionWithStatus(leasing.tx(), ApplicationStatus.SUCCEEDED));
  ...
  void leasingInBlock() throws IOException, NodeException {
    ...
    TransactionWithStatus leaseTxInBlock =
        node.getBlock(leaseTxInfo.height()).transactions().stream()
            .filter(t -> t.tx().id().equals(leaseTx.id()))
            .findFirst()
            .orElseThrow(AssertionError::new);
    TransactionWithStatus leaseTxInBlocksSeq =
        node.getBlocks(leaseTxInfo.height(), leaseTxInfo.height()).get(0).transactions().stream()
            .filter(t -> t.tx().id().equals(leaseTx.id()))
            .findFirst()
            .orElseThrow(AssertionError::new);
    ...
    assertThat(leaseTxInBlock)
        .isEqualTo(new TransactionWithStatus(leaseTx, ApplicationStatus.SUCCEEDED));

    // 2. cancel

    LeaseCancelTransaction cancelTx =
        LeaseCancelTransaction.builder(leaseTx.id()).getSignedWith(alice);
    TransactionInfo cancelTxInfo = node.waitForTransaction(node.broadcast(cancelTx).id());

    TransactionWithStatus cancelTxInBlock =
        node.getBlock(cancelTxInfo.height()).transactions().stream()
            .filter(t -> t.tx().id().equals(cancelTx.id()))
            .findFirst()
            .orElseThrow(AssertionError::new);
    TransactionWithStatus cancelTxInBlocksSeq =
        node.getBlocks(cancelTxInfo.height(), cancelTxInfo.height()).get(0).transactions().stream()
            .filter(t -> t.tx().id().equals(cancelTx.id()))
            .findFirst()
            .orElseThrow(AssertionError::new);
    ...
    assertThat(cancelTxInBlock)
        .isEqualTo(new TransactionWithStatus(cancelTx, ApplicationStatus.SUCCEEDED));
  }
}
