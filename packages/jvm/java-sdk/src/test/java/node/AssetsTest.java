...
import io.decentralchain.sdk.AssetBalance;
import io.decentralchain.sdk.AssetDistribution;
...
public class AssetsTest extends BaseTestWithNodeInDocker {
  ...
  void distribution() throws IOException, NodeException {
    ...
    AssetId assetId =
        node.broadcast(
                IssueTransaction.builder("Asset", IntStream.range(1, recipientsNumber + 1).sum(), 2)
                    .getSignedWith(alice))
            .assetId();
    node.waitForTransaction(assetId);

    List<Transfer> transfersToDistribute =
        IntStream.range(1, recipientsNumber + 1)
            .mapToObj(
                amount ->
    ...
    List<MassTransferTransaction> massTransferTxs =
        transfersToDistribute.stream()
            .collect(groupingBy(it -> transfersCounter.getAndIncrement() / 100))
            .values()
            .stream()
            .map(
                transfers ->
                    MassTransferTransaction.builder(transfers)
                        .assetId(assetId)
                        .getSignedWith(alice))
            .collect(toList());

    List<Id> massTransferTxIds =
        massTransferTxs.stream().map(MassTransferTransaction::id).collect(toList());

    for (MassTransferTransaction massTransferTx : massTransferTxs) node.broadcast(massTransferTx);
    ...
    AssetDistribution distributionPage2 =
        node.getAssetDistribution(
            assetId, node.getHeight() - 1, 1000, distributionPage1.lastItem());
  ...
  void balance() throws IOException, NodeException {
    ...
    AssetId assetId1 =
        node.broadcast(IssueTransaction.builder("Asset 1", 10000000, 2).getSignedWith(alice))
            .assetId();
    node.waitForTransaction(assetId1);

    AssetId assetId2 =
        node.broadcast(IssueTransaction.builder("Asset 2", 10000000, 2).getSignedWith(alice))
            .assetId();
  ...
  void balanceWithAssetsFilter() throws IOException, NodeException {
    ...
    AssetId assetId3 =
        node.broadcast(IssueTransaction.builder("Asset 3", 10000000, 2).getSignedWith(alice))
            .assetId();
