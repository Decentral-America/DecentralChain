...
public class BlockMockTest {
  private final Node node =
      new Node(Profile.LOCAL, mockHttpClient("src/test/resources/stub/addressesD.json"));
  ...
  @Test
  void readBlocksHeadersWithFinalizationVoting()
      throws NodeException, IOException, InterruptedException {
    mockGBlockHeadersRs(node, 16, "src/test/resources/stub/blocks/block_with_voting.json");
    ...
    assertEquals("5VjPUXb4NU9QQFoTsEET2xM9ncJfVzjQYZwpUQENStMH", bh.vrf().encoded());
    ...
    assertEquals("3LKirJGRvMcdaachGNEuWVSyGbpzWKcagrjsQQuaYXwq", ce.finalizedBlockId().encoded());
    ...
    assertEquals("BCuRAvrPqngYBKvJ3HKERz3nZZQRKEUMY81FJCEQPfNV", bh.generatorPublicKey().encoded());
    assertEquals("HwLttQ7xwStDavkXbBtfaeUy3QoKJMbCbTkNEezivEL6", bh.stateHash().encoded());
    assertEquals(3, bh.rewardShares().size());
  }

  @Test
  public void readChallengedBlocksHeaders()
      throws IOException, NodeException, InterruptedException {
    mockGBlockHeadersRs(node, 42, "src/test/resources/stub/blocks/challenged_block.json");
    ...
    assertEquals("3Fgtiv5L5q4CXFfwuAfbkjy9ppehkNzjbEG", ch.generator().encoded());
    assertEquals("5FUsFwB6b8mE61VMbSB6X7m23NTy4yN8f1fApXo8tZs7", ch.generatorPublicKey().encoded());
    assertEquals(-1L, ch.desiredReward());
    assertEquals("11111111111111111111111111111111", ch.stateHash().encoded());
