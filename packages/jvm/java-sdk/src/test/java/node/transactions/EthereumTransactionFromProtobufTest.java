...
public class EthereumTransactionFromProtobufTest {
  ...
  public void readEthereumTransferTransactionFromPb() {
    byte[] decode =
        Base58.decode(
            "2mXfCNgXmQhpaR14kr5Vg1s7BBDPCx9jYHLAK18narLsRYJEUypk"
                + "hYKR7oJR8xne4cBP13NLZvAdWKsvmMFvagrQR9yUdP8PVZjDDmyLEwDBJDdKjWNz1f"
                + "zzK3nao3BWFubTEk6q21zUYJw9FVse6A5Qm14QLmREga");
    SignedTransaction signedTransaction =
        SignedTransaction.newBuilder().setEthereumTransaction(ByteString.copyFrom(decode)).build();

    EthereumTransaction ethTransferTransaction =
        EthereumTransaction.transferTxfromProtobuf(signedTransaction);

    assertEquals(
        "Ba4pFx78Ueg3j6CZqjhuBdg5cjxTwTzJJSS6GPpH3Cn4", ethTransferTransaction.id().encoded());
    ...
    EthereumTransaction.Transfer payload =
        (EthereumTransaction.Transfer) ethTransferTransaction.payload();
  ...
  public void readEthereumInvokeScriptTransactionWithPaymentFromPb() throws IOException {
    SignedTransaction signedTransaction =
        SignedTransaction.newBuilder()
            .setEthereumTransaction(ByteString.copyFrom(Base58.decode(signedInvokeTxWithPayments)))
            .build();

    EthereumTransaction ethInvokeScriptTransaction =
        EthereumTransaction.invokeScriptTxfromProtobuf(
            signedTransaction, buildInvokeScriptWithPaymentsMetadata());

    assertEquals(
        "HDuM4GXQrBpHeCnk4WDMkEkktwEi4caniMA61gcGsEUh", ethInvokeScriptTransaction.id().encoded());
  ...
  public void readEthereumInvokeScriptTransactionWithoutPaymentFromPb() throws IOException {
    ...
    EthereumTransaction ethInvokeScriptTransaction =
        EthereumTransaction.invokeScriptTxfromProtobuf(
            signedTransaction, buildInvokeScriptWithoutPaymentsMetadata());

    assertEquals(
        "HmwNwr7189HdwRHCYYcEq8mmNQjjCooqnjuSbCWccHfg", ethInvokeScriptTransaction.id().encoded());
    assertEquals(18, ethInvokeScriptTransaction.type());
    assertEquals(1, ethInvokeScriptTransaction.version());
    assertEquals(83, ethInvokeScriptTransaction.chainId());
    assertEquals(
  ...
  private TransactionMetadata buildInvokeScriptWithoutPaymentsMetadata() {
    return TransactionMetadata.newBuilder()
        .setEthereum(
            EthereumMetadata.newBuilder()
                .setSenderPublicKey(
                    ByteString.copyFrom(
                        Base58.decode(
                            "5BcWEhVZFuvMhs8DRF1C8GLSbXvobkfaaRvANa86MboLSQLxC5989Zgo3Djp8WwWKo3JshYP39NnBAJRqTCXZ5Qd")))
                .setInvoke(
                    TransactionMetadata.InvokeScriptMetadata.newBuilder()
                        .setDAppAddress(
                            ByteString.copyFrom(
            ...
            EthereumMetadata.newBuilder()
                ...
                .setInvoke(
                    TransactionMetadata.InvokeScriptMetadata.newBuilder()
                        ...
                        .addArguments(
                            0, Argument.newBuilder().setStringValue("Первая из строк").build())
                        .addArguments(1, Argument.newBuilder().setIntegerValue(1).build())
                        .addArguments(2, Argument.newBuilder().setBooleanValue(true).build())
                        .addArguments(
                            3,
                            Argument.newBuilder()
                                .setBinaryValue(ByteString.copyFrom(Base64.decode("4BD+OLA=")))
                                .build())
                        .addArguments(
                            4, Argument.newBuilder().setStringValue("Вторая из строк").build())
                        ...
                        .addArguments(
                            ...
                            Argument.newBuilder()
                                .setBinaryValue(
                                    ByteString.copyFrom(Base64.decode("AUWrPlBPQlWXsn4=")))
                                .build())
                        .addArguments(
                            8, Argument.newBuilder().setStringValue("Третья из строк").build())
                        ...
                        .addArguments(
                            ...
                            Argument.newBuilder()
                                .setBinaryValue(
                                    ByteString.copyFrom(Base64.decode("pimpshwamp21312390AA")))
                                .build())
                        .addArguments(12, Argument.newBuilder().setIntegerValue(1).build())
                        .addArguments(
                            13, Argument.newBuilder().setIntegerValue(12354634543L).build())
                        .addPayments(
                            0,
                            Amount.newBuilder()
  ...
  private final String signedInvokeTxWithoutPayments =
      "i3fxSybQfxBBG4xLgJpiWw9zGKHxDbaLwSWnS6Fd1A6dH3wRFRZ2w"
  ...
  private final String signedInvokeTxWithPayments =
      "TW82GbQC6tHsjHt9Y8hJNgHh57QDw3y44ggy2M2gKeqCZj2UjK1teNWcMJbLoETMP39"
