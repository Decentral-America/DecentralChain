...
public class TransactionsInfoTest extends BaseTestWithNodeInDocker {
  ...
  void transferTransactionInfo() throws IOException, NodeException {
    ...
    TransactionInfo commonInfo = node.getTransactionInfo(tx.id());
    TransferTransactionInfo txInfo =
        node.getTransactionInfo(tx.id(), TransferTransactionInfo.class);
  ...
  void reissueTransactionInfo() throws IOException, NodeException {
    ...
    AssetId assetId =
        node.waitForTransaction(
            ...
            .tx()
            .assetId();
    ReissueTransaction tx =
        ReissueTransaction.builder(Amount.of(1000, assetId)).getSignedWith(alice);
    node.waitForTransaction(node.broadcast(tx).id());
  ...
  void exchangeTransactionInfo() throws IOException, NodeException {
    ...
    ExchangeTransaction tx =
        ExchangeTransaction.builder(
                buy, sell, amount.value(), price.value(), matcherFee, matcherFee)
            .getSignedWith(alice);
    node.waitForTransaction(node.broadcast(tx).id());

    TransactionInfo commonInfo = node.getTransactionInfo(tx.id());
    ExchangeTransactionInfo txInfo =
        node.getTransactionInfo(tx.id(), ExchangeTransactionInfo.class);
  ...
  void leaseCancelTransactionInfo() throws IOException, NodeException {
    ...
    TransactionInfo commonInfo = node.getTransactionInfo(tx.id());
    LeaseCancelTransactionInfo txInfo =
        node.getTransactionInfo(tx.id(), LeaseCancelTransactionInfo.class);
  ...
  void createAliasTransactionInfo() throws IOException, NodeException {
    ...
    CreateAliasTransaction tx =
        CreateAliasTransaction.builder(alias.toString()).getSignedWith(alice);
    node.waitForTransaction(node.broadcast(tx).id());

    TransactionInfo commonInfo = node.getTransactionInfo(tx.id());
    CreateAliasTransactionInfo txInfo =
        node.getTransactionInfo(tx.id(), CreateAliasTransactionInfo.class);
  ...
  void massTransferTransactionInfo() throws IOException, NodeException {
    ...
    MassTransferTransaction tx =
        MassTransferTransaction.builder(Transfer.to(bob, 1000)).getSignedWith(alice);
    node.waitForTransaction(node.broadcast(tx).id());

    TransactionInfo commonInfo = node.getTransactionInfo(tx.id());
    MassTransferTransactionInfo txInfo =
        node.getTransactionInfo(tx.id(), MassTransferTransactionInfo.class);
  ...
  void dataTransactionInfo() throws IOException, NodeException {
    ...
    DataTransaction tx =
        DataTransaction.builder(StringEntry.as("str", alice.address().toString()))
            .getSignedWith(alice);
    node.waitForTransaction(node.broadcast(tx).id());
  ...
  void setScriptTransactionInfo() throws IOException, NodeException {
    ...
    TransactionInfo commonInfo = node.getTransactionInfo(tx.id());
    SetScriptTransactionInfo txInfo =
        node.getTransactionInfo(tx.id(), SetScriptTransactionInfo.class);
  ...
  void sponsorFeeTransactionInfo() throws IOException, NodeException {
    ...
    TransactionInfo commonInfo = node.getTransactionInfo(tx.id());
    SponsorFeeTransactionInfo txInfo =
        node.getTransactionInfo(tx.id(), SponsorFeeTransactionInfo.class);
  ...
  void setAssetScriptTransactionInfo() throws IOException, NodeException {
    ...
    SetAssetScriptTransaction tx =
        SetAssetScriptTransaction.builder(assetId, script).getSignedWith(alice);
    node.waitForTransaction(node.broadcast(tx).id());

    TransactionInfo commonInfo = node.getTransactionInfo(tx.id());
    SetAssetScriptTransactionInfo txInfo =
        node.getTransactionInfo(tx.id(), SetAssetScriptTransactionInfo.class);
  ...
  void invokeScriptTransactionInfo() throws IOException, NodeException {
    ...
    Base64String script =
        node.compileScript(
            ...
            .script();
    node.waitForTransaction(
        node.broadcast(SetScriptTransaction.builder(script).getSignedWith(bob)).id());
    ...
    TransactionInfo commonInfo = node.getTransactionInfo(tx.id());
    InvokeScriptTransactionInfo txInfo =
        node.getTransactionInfo(tx.id(), InvokeScriptTransactionInfo.class);
  ...
  void updateAssetInfoTransactionInfo() throws IOException, NodeException {
    ...
    UpdateAssetInfoTransaction tx =
        UpdateAssetInfoTransaction.builder(assetId, "New Asset", "New description")
            .getSignedWith(alice);
    node.waitForTransaction(node.broadcast(tx).id());

    TransactionInfo commonInfo = node.getTransactionInfo(tx.id());
    UpdateAssetInfoTransactionInfo txInfo =
        node.getTransactionInfo(tx.id(), UpdateAssetInfoTransactionInfo.class);
  ...
  void multipleTransactionsInfoWithSpecifiedType() throws NodeException, IOException {
    ...
    List<CreateAliasTransactionInfo> txsInfo =
        node.getTransactionsInfo(asList(aliasTxId1, aliasTxId2), CreateAliasTransactionInfo.class);
    ...
    assertThrows(
        ...
        () ->
            node.getTransactionsInfo(
                asList(aliasTxId1, issueTxId), CreateAliasTransactionInfo.class),
        "Cannot cast io.decentralchain.sdk.info.IssueTransactionInfo to io.decentralchain.sdk.info.CreateAliasTransactionInfo");
  }
}
