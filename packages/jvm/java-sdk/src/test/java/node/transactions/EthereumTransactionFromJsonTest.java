...
public class EthereumTransactionFromJsonTest {
  ...
  private final Node node =
      new Node(Profile.LOCAL, mockHttpClient("src/test/resources/stub/addresses.json"));

  private static final byte PRIVATE_NODE_CHAIN_ID = 'R';

  public EthereumTransactionFromJsonTest()
      throws NodeException, IOException, InterruptedException {}

  @Test
  void readEthereumTransferTransactionByIdTest()
      throws NodeException, IOException, InterruptedException {
    mockTransactionInfoRs(
        node,
    ...
    EthereumTransactionInfo ethTransferTxInfo =
        (EthereumTransactionInfo)
            node.getTransactionInfo(new Id("Ba4pFx78Ueg3j6CZqjhuBdg5cjxTwTzJJSS6GPpH3Cn4"));
    ...
    assertEquals(
        "3M86rqkCp9hYim1cEbRC8MiajcktM16ogVx",
        ethTransferTx.sender().address(PRIVATE_NODE_CHAIN_ID).encoded());
    assertEquals(
  ...
  @Test
  public void readEthereumTransferTransactionFromBlockTest()
      throws NodeException, IOException, InterruptedException {
    mockGetBlockRs(
        node, 1043438, "src/test/resources/stub/txs/eth/eth_transfer_tx_from_block.json");
  ...
  @Test
  void readEthereumInvokeTransactionByIdTest()
      throws NodeException, IOException, InterruptedException {
    mockTransactionInfoRs(
        node,
    ...
    EthereumTransactionInfo ethInvokeTxInfo =
        (EthereumTransactionInfo)
            node.getTransactionInfo(new Id("CWuFY42te67sLmc5gwt4NxwHmFjVfJdHkKuLyshTwEct"));
    ...
    assertEquals(
        "3MCMsdDqmhoz4wFHus4XKD1xJ8SAsimgBTW",
        ethInvokeTx.sender().address(PRIVATE_NODE_CHAIN_ID).encoded());
    assertEquals(
        "2HAk5dPx7Jx7fwbehqA9JRM9de9E7ZXtxVA2u92vYAp9ttZQiVgChPwBdoJ7ck2wcXmgfGxiAK9a6PPmmtEZmhvd",
  ...
  @Test
  void readEthereumInvokeTransactionFromBlockTest()
      throws NodeException, IOException, InterruptedException {
    mockGetBlockRs(node, 1043725, "src/test/resources/stub/txs/eth/eth_invoke_tx_from_block.json");
  ...
  @Test
  void readExchangeTransactionInfoWithEthereumSignatureTest()
    ...
    ExchangeTransactionInfo exchangeTxInfo =
        node.getTransactionInfo(
            new Id("3ZPyxs4p7abkj5cf43pUkaHaMPRuBaWC89RX93d2AB6R"), ExchangeTransactionInfo.class);
    ...
    assertEquals(
        ...
        exchangeTx.sender().address(PRIVATE_NODE_CHAIN_ID).encoded());
    assertEquals("CsdeJPgJebkW4tsvAaWtfzNvaFd7nWJUGiHrqrtEGN9p", exchangeTx.sender().encoded());
    assertEquals(
        "3vbCSYNC6T3BrooeULFpHewphHKLV8p6cDz9NFYQEpkGzA8HCaL7EDTYRYEyNs26AiqS1gdHnxb4H6AYUqdmH8pd",
    ...
    assertEquals(
        "3M6zKVa3cshzkPRRobLaZjzkrT4fHtuSn3Z",
        order1.sender().address(PRIVATE_NODE_CHAIN_ID).encoded());
    ...
    assertEquals(
        "CDyHKz5S5dmnBceTxix1cGQpetXAwWVXKiez2TpvpVLw", order1.assetPair().left().encoded());
    ...
    assertEquals(
        "5qneeJ2va7M8uKhy93ycqWeWstNb4FDSSeV86ZHgFrykDpffcqdEQrq1Ux8XHP18a3eMwA6HHCzr19sdQDywfYNU",
    ...
    assertEquals(
        "3M2jsLHQgaewRmP6BChR2LEjpsef2VZSixf",
        order2.sender().address(PRIVATE_NODE_CHAIN_ID).encoded());
    assertEquals(
    ...
    assertEquals(
        "CDyHKz5S5dmnBceTxix1cGQpetXAwWVXKiez2TpvpVLw", order2.assetPair().left().encoded());
  ...
  @Test
  void readExchangeTransactionWithEthereumSignatureFormBlockTest()
      throws IOException, NodeException, InterruptedException {
    mockGetBlockRs(
        node, 1099637, "src/test/resources/stub/txs/eth/exchange_tx_with_eth_sign_from_block.json");
  ...
  private static final String transferBytes =
      "0xf87486017cab97da1d8502540be400830186a094c01187f4ae820a0c956c48c06ee"
  ...
  private static final String invokeBytes =
      "0xf9011186017cac99be168502540be4008307a120940ea8e14f313237aac31995f9"
