...
Java JNI bindings for [wavesplatform/zwaves](https://github.com/wavesplatform/zwaves) —
...
Forked from `wavesplatform/zwaves` at commit `d4546dbb` with full upstream history.
...
boolean result = com.wavesplatform.zwaves.bls12.Groth16.verify(vk, proof, inputs);

// BN256 (Ethereum / Zcash Groth16)
boolean result = com.wavesplatform.zwaves.bn256.Groth16.verify(vk, proof, inputs);
```
...
│   └── javalib/src/main/
│       ├── java/com/wavesplatform/zwaves/
│       │   ├── bls12/Groth16.java
│       │   ├── bn256/Groth16.java
│       │   ├── JNILibrary.java   # hawtjni loader (EPL-1.0)
│       │   └── ZWavesJNILibrary.java
...
This package tracks `wavesplatform/zwaves` via `git subtree`:

```bash
# Pull upstream updates:
git subtree pull --prefix=packages/jvm/zwaves \
...
The upstream Rust codebase (`wavesplatform/zwaves`) is MIT-licensed.
