...
public class OrderTest {
    ...
    static Amount dccFee = Amount.of(Order.MIN_FEE + 1);
    ...
    static Stream<Arguments> ordersProvider() {
        return Stream.of(
                arguments(1, OrderType.BUY, dccFee, Id.as("Bea2BQsswtozYBNCtmqMAvFN62ePe8y8vvErxB9EFzKg"),
                ...
                arguments(1, OrderType.SELL, dccFee, Id.as("CsMYW2s8FwktKZKFpsoteHGKSqyrL2ezveodQ4XVjGEP"),
                ...
                arguments(2, OrderType.SELL, dccFee, Id.as("2d81nLUDgsmr9HvGd7RekXcU9FWBD5gbJA2rtXgLroek"),
                ...
                arguments(3, OrderType.BUY, dccFee, Id.as("CVSxQaBe4cpisdVgRJGZ4DnmFDjpNnzgi6rV2DKdPVPh"),
                ...
                arguments(4, OrderType.SELL, dccFee, Id.as("3HqWSTfN4E7XgmUrJvmfg1DLLhFxXLDdzcCwn2ngomB8"),
                ...
                arguments(4, OrderType.BUY, dccFee, Id.as("7ZGMgJBRXGr96TSS2nkgA4WRTTjHpci5BSFUX4FFsEwE"),
    ...
    void order(int version, OrderType type, Amount fee, Id expectedId, List<Proof> proofs,
        ...
        assertAll("Order created via builder must be equal to expected bytes",
                () -> Assertions.assertThat(builtOrder.bodyBytes()).isEqualTo(expectedBody),
                () -> Assertions.assertThat(builtOrder.id()).isEqualTo(expectedId),
                () -> Assertions.assertThat(builtOrder.toBytes()).isEqualTo(expectedBytes)
        );

        Order constructedOrder = new Order(sender, type, amount, price, matcher.publicKey(),
                DccConfig.chainId(), fee, timestamp, expiration, version, proofs);

        assertAll("Orders created via builder and constructor are equal",
                () -> Assertions.assertThat(builtOrder.bodyBytes()).isEqualTo(constructedOrder.bodyBytes()),
                () -> Assertions.assertThat(builtOrder.id()).isEqualTo(constructedOrder.id()),
                () -> Assertions.assertThat(builtOrder.toBytes()).isEqualTo(constructedOrder.toBytes())
        );
        ...
        assertAll("Order must be deserializable from expected bytes",
                ...
                () -> Assertions.assertThat(deserOrder.amount()).isEqualTo(amount),
                () -> Assertions.assertThat(deserOrder.price()).isEqualTo(price),
                () -> assertThat(deserOrder.expiration()).isEqualTo(expiration),

                () -> Assertions.assertThat(deserOrder.version()).isEqualTo(version),
                () -> Assertions.assertThat(deserOrder.chainId()).isEqualTo(DccConfig.chainId()),
                () -> Assertions.assertThat(deserOrder.sender()).isEqualTo(sender),
                () -> Assertions.assertThat(deserOrder.fee()).isEqualTo(fee),
                () -> Assertions.assertThat(deserOrder.timestamp()).isEqualTo(timestamp),
                () -> Assertions.assertThat(deserOrder.proofs()).isEqualTo(proofs),

                () -> Assertions.assertThat(deserOrder.bodyBytes()).isEqualTo(expectedBody),
                () -> Assertions.assertThat(deserOrder.toBytes()).isEqualTo(expectedBytes),
                () -> Assertions.assertThat(deserOrder.id()).isEqualTo(expectedId)
        );
        ...
        Assertions.assertThat(JsonSerializer.JSON_MAPPER.readTree(Order.fromJson(expectedJson).toJson()))
