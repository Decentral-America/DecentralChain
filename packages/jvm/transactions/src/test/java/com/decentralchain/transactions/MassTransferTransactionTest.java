...
public class MassTransferTransactionTest {
    ...
    static Stream<Arguments> transactionsProvider() {
        ...
        List<Transfer> address = asList(
                Transfer.to(Address.from(DccConfig.chainId(), sender), maxAmount),
                Transfer.to(Address.from(DccConfig.chainId(), sender), minAmount)
        );
        ...
        return Stream.of(
                arguments(1, address, AssetId.DCC, Base58String.empty(), Id.as("9BctskM13RG8ZpSnCexp9ZyE2ktdnK38qSMQmjxZcJTS"),
                ...
                arguments(2, address, AssetId.DCC, Base58String.empty(), Id.as("ByZGfrDr5YLvUCHWm2Rn4N2NrvYTkLjckGejw33aAFa3"),
    ...
    void massTransferTransaction(int version, List<Transfer> transfers, AssetId assetId, Base58String attachment,
        ...
        assertAll("Tx must be deserializable from expected bytes",
                () -> Assertions.assertThat(deserTx.transfers()).isEqualTo(transfers),
                () -> Assertions.assertThat(deserTx.assetId()).isEqualTo(assetId),
                () -> Assertions.assertThat(deserTx.attachment()).isEqualTo(attachment),
