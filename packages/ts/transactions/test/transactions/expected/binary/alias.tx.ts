export const aliasBinaryTx = {
  'Create Alias TX (type 10 v2)  case 1 max value': {
    Bytes:
      'CgKNj7KNwHV8CsVGLbpgRgDsF4sHe1SAkroj0YoxaGM3dAAiAkwAHl9yaWNoLWFjY291bnQud2l0aEAzMF9zeW1ib2xzX3//////////AAABfeG6aK0=',
    Json: {
      alias: '_rich-account.with@30_symbols_',
      // chainId 76 ('L') is baked into the Bytes binary reference — set explicitly
      // so this test vector documents a chain-ID-76 alias, not the DCC default (63).
      chainId: 76,
      fee: '9223372036854775807',
      feeAssetId: null,
      id: '9K3CW7EyoNrFg4ptE4WkN2HTK8emXYi4RkPciFGor5Jg',
      proofs: [
        'zWFQFBCDZqSSaT3eRvcjGPNkbJSjrD68LKC2EckpTbHiUyvmqiuRu3scCTEJgFS4b3amwe4bGpiPCmr3qWEwAJC',
      ],
      sender: '3MsX9C2MzzxE4ySF5aYcJoaiPfkyxZMg4cW',
      senderPublicKey: 'AXbaBkJNocyrVpwqTzD4TpUY8fQ6eeRto9k1m2bNCzXV',
      timestamp: 1640169629869,
      type: 10,
      version: 2,
    },
  },
  'Create Alias TX (type 10 v2)  case 2 min value': {
    Bytes:
      'CgKNj7KNwHV8CsVGLbpgRgDsF4sHe1SAkroj0YoxaGM3dAAiAkwAHl9yaWNoLWFjY291bnQud2l0aEAzMF9zeW1ib2xzXwAAAAAAAYagAAABfZ90YLg=',
    Json: {
      alias: '_rich-account.with@30_symbols_',
      // chainId 76 ('L') is baked into the Bytes binary reference — set explicitly
      // so this test vector documents a chain-ID-76 alias, not the DCC default (63).
      chainId: 76,
      fee: 100000,
      feeAssetId: null,
      id: 'BrasfMP34YYDXvJsXgRq7XojyfNGpU4KDLoZ5BrQ93s9',
      sender: '3FS7VSHbPe4p3tW1hBqHD3aMobgrsHf2oqX',
      senderPublicKey: 'AXbaBkJNocyrVpwqTzD4TpUY8fQ6eeRto9k1m2bNCzXV',
      timestamp: 1639057744056,
      type: 10,
      version: 2,
      // "signature":"3q5YSei9kcJE789fCG3Aw5aHpeme6z78jt62apjnx8PctSSvDLzpKfpPEjFWLBnSZCNUHiF3ACJZxJNFxCf5hN1Q",
      // "proofs":["3q5YSei9kcJE789fCG3Aw5aHpeme6z78jt62apjnx8PctSSvDLzpKfpPEjFWLBnSZCNUHiF3ACJZxJNFxCf5hN1Q"]
    },
  },
};
