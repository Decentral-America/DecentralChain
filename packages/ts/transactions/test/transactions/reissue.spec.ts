...
describe('reissue', () => {
  ...
  it('Should not create with negative fee', () => {
    expect(() => reissue({ ...reissueMinimalParams, fee: -1 }, stringSeed)).toThrowError(
  ...
  it('Should not create with zero quantity', () => {
    expect(() => reissue({ ...reissueMinimalParams, quantity: 0 }, stringSeed)).toThrowError(
      errorMessageByTemplate('quantity', 0),
    );
  });

  it('Should not create with negative quantity', () => {
    expect(() => reissue({ ...reissueMinimalParams, quantity: -1 }, stringSeed)).toThrowError(
      errorMessageByTemplate('quantity', -1),
    );
  });

  it('Should not create with empty assetId', () => {
    expect(() => reissue({ ...reissueMinimalParams, assetId: '' }, stringSeed)).toThrowError(
