...
import java.math.{BigDecimal as BD, BigInteger}
  ...
  override def base16DecodeImpl(input: String): Either[String, Array[Byte]] = {
    ...
    if (size % 2 == 1) {
      ...
      input.toSeq
        .sliding(2, 2)
        .map(s =>
          Try(Integer.parseInt(s.unwrap, 16).toByte).toEither.left.map(_ => s"${s.unwrap} isn't base16/hex digit")
        )
