...
object Types {
  ...
  case class TYPEPARAM(char: Byte) extends PARAMETERIZED with SINGLE {
    override def toString: String = char.toChar.toString
  }
  case class PARAMETERIZEDLIST(t: TYPE) extends PARAMETERIZED with SINGLE {
    override def toString: String = s"List[$t]"
  }
  case class PARAMETERIZEDTUPLE(t: List[TYPE]) extends PARAMETERIZED with SINGLE {
  ...
  case class CASETYPEREF(
      override val name: String,
      override val fields: List[(String, FINAL)],
      hideConstructor: Boolean = false
  ) extends REAL {
    override def typeList: List[REAL]      = List(this)
    override def equals(obj: Any): Boolean =
      obj match {
  ...
  implicit class TypeExt(l1: FINAL) {
    def equivalent(l2: FINAL): Boolean = (l1, l2) match {
      ...
      case (l1: UNION, l2: UNION) =>
        ...
        (l1.unfold.typeList
          .sortBy(_.name)
          .zip(l2.unfold.typeList.sortBy(_.name)))
