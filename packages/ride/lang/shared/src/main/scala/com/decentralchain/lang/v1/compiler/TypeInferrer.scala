...
object TypeInferrer {
  ...
  def apply(
      seq: Seq[(FINAL, TYPE)],
      knownTypes: Map[String, FINAL] = Map.empty
  ): Either[String, Map[TYPEPARAM, FINAL]] = {
  ...
  private def checkTuplesCommonType(matchingTuples: List[TUPLE], commonTuple: TUPLE): Boolean =
    (groupByPosition(matchingTuples.map(_.types))
      .zip(commonTuple.types))
      .forall {
  ...
  private def matchTypes(
    ...
    (placeholder, argType) match {
      ...
      case (PARAMETERIZEDLIST(innerTypeParam), LIST(t)) =>
      ...
      case (PARAMETERIZEDUNION(l), _)                     =>
        ...
        if (concretes >= UNION.create(argType.typeList)) Right(None)
          ...
          val error =
            s"Can't resolve correct type for parameterized $placeholder, actual: $argType".asLeft[Option[MatchResult]]
          parameterized
