...
const getAssetPair = (data: IClientExchangeOrder) => ({
...
export const exchange = (data: IClientExchange): TWithPartialFee<ExchangeTransaction<string>> => {
  const base = {
...
export interface IClientExchange extends IDefaultGuiTx<typeof TYPES.EXCHANGE> {
  buyOrder: IClientExchangeOrder;
  sellOrder: IClientExchangeOrder;
  price: TLong;
  amount: TLong;
...
export interface IClientExchangeOrder {
  version: number;
