...
describe('lease', () => {
  ...
  it('Should not create with negative fee', () => {
    expect(() => lease({ ...leaseMinimalParams, fee: -1 }, stringSeed)).toThrowError(
  ...
  it('Should create with zero amount', () => {
    expect(() => lease({ ...leaseMinimalParams, amount: 0 }, stringSeed)).toThrowError(
      errorMessageByTemplate('amount', 0),
    );
  });

  it('Should not create with negative amount', () => {
    expect(() => lease({ ...leaseMinimalParams, amount: -1 }, stringSeed)).toThrowError(
      errorMessageByTemplate('amount', -1),
    );
  });

  it('Should not create with empty recepient', () => {
    expect(() => lease({ ...leaseMinimalParams, recipient: '' }, stringSeed)).toThrowError(
      errorMessageByTemplate('recipient', ''),
    );
  });
