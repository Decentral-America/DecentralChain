export const sponsorshipTx = {
  'SponsorFee TX (type 14 v2)  case 1': {
    Bytes:
      'CCESIJL4Cg7m8oV+sUn/3SQWcJnIDvBjXYh3TIjvckIJrSwvGgQQoI0GIKjxkL2mLygCkgcuCiwKIOQt6VHG7an7FIw6Rvt44jtBL/LIHaogX6oHdthqs7EgEP//////////fw==',
    Json: {
      assetId: 'GMiZSiRUd2kWrc2PDVWGGE2bB219wp2CHCBUipeJkvVm',
      chainId: 33,
      fee: 100000,
      feeAssetId: null,
      id: 'A8Ht1dxzF831Np323RQVHhLXswwcEHGHr8mLgixPZQRs',
      minSponsoredAssetFee: '9223372036854775807',
      proofs: [
        '5pxTMmDPuJ8ARSATTKcDcTMXHHESdnfPZv6Q3XNVWi4xsvUHeJuVA4SmHnUBGEzmpMqupsJuWou6hVqvD1iwQwLJ',
      ],
      sender: '31XWSzrLgPPRFHTJk9MhTgcXzT4CPhMGh1D',
      senderPublicKey: 'Athtgb7Zm9V6ExyAzAJM1mP57qNAW1A76TmzXdDZDjbt',
      timestamp: 1625236453544,
      type: 14,
      version: 2,
    },
  },
  'SponsorFee TX (type 14 v2)  case 2': {
    Bytes:
      'CCESII2Pso3AdXwKxUYtumBGAOwXiwd7VICSuiPRijFoYzd0GgoQ//////////9/IM+cubreLygCkgcmCiQKIOQt6VHG7an7FIw6Rvt44jtBL/LIHaogX6oHdthqs7EgEAE=',
    Json: {
      assetId: 'GMiZSiRUd2kWrc2PDVWGGE2bB219wp2CHCBUipeJkvVm',
      chainId: 33,
      fee: '9223372036854775807',
      feeAssetId: null,
      id: 'DpUDq9jhkkUoVWa8nFMXuBZgpWbfjCVUctEe7pcD2u7V',
      minSponsoredAssetFee: 1,
      proofs: [
        '3x3AjsuqLaF52NRBcnCUBhh3MBE97GkLQ6EYTrKxLAY7ccwHhH4na6VzWqWJmEQCjJUVLbwdmyfEBpBmtJiAw8rK',
      ],
      sender: '31MHNyVAJnkbPGL8fL9d45BqGa2qrN5ERK7',
      senderPublicKey: 'AXbaBkJNocyrVpwqTzD4TpUY8fQ6eeRto9k1m2bNCzXV',
      timestamp: 1640263208527,
      type: 14,
      version: 2,
    },
  },
};
