    ...
    ok<ValidationError, string[]>(data)
      ...
      .chain((d) =>
        d.length === 2
          ...
          : error<ValidationError, string[]>(err('Cursor length is not equals to 2')),
      )
      .chain((d) => {
        const s = d[1];
        if (isSortOrder(s)) {
          return ok<ValidationError, [BigNumber, SortOrder]>([new BigNumber(d[0]), s]);
        } else {
          return error<ValidationError, [BigNumber, SortOrder]>(err('Sort is not valid'));
        }
      })
      .map(([uid, sort]) => ({
        sort,
