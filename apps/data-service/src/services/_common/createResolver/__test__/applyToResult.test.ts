...
import { Error, /* Result */ Ok } from 'folktale/result';
...
describe('Application of functions to db results', () => {
  describe('validation', () => {
    ...
    describe('Mget', () => {
      ...
      it('empty results', () => {
        ...
        expect(applyValidation.mget(validate)(results)).toEqual(Ok<AppError, Maybe<number>[]>([]));
      });
    });
    ...
    describe('Search', () => {
      it('valid results', () => {
        const results = [2, 2];
        expect(applyValidation.search(validate)(results)).toEqual(Ok<AppError, number[]>(results));
      });
      it('invalid results', () => {
        const results = [2, 3];
      ...
      it('empty results', () => {
        ...
        expect(applyValidation.search(validate)(results)).toEqual(Ok<AppError, number[]>([]));
      });
    });
  });
  ...
  describe('transformation', () => {
    ...
    describe('Mget', () => {
      it('valid result', () => {
        expect(applyTransformation.mget(transform)([maybeOf(2), empty(), maybeOf(3)])).toEqual([
          '2',
          null,
          '3',
        ]);
