...
const select = (jestSpy) => ({
  ...
  get lastCall() {
    ...
    if (!get || !apply) throw new Error('Last proxy interaction was not a method call.');
  ...
  get lastField() {
    if (jestSpy.mock.calls.length < 1) throw new Error('No proxy interactions so far.');

    const { get } = jestSpy.mock.calls.slice(-1)[0][0];

    if (!get) throw new Error('Last proxy interaction was not a field resolve.');
