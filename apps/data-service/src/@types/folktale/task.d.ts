declare module 'folktale/concurrency/task' {
  ...
  export type Computation<A, B> = (resolver: {
  ...
  export interface Task<A, B> extends Matchable {
    ...
    apply<C, D>(f: Task<C, D>): B extends (d: D) => infer R ? Task<A | C, R> : unknown;
  ...
  function waitAny<A1, B1, A2, B2>(ts: [Task<A1, B1>, Task<A2, B2>]): Task<A1 | A2, B1 | B2>;
  ...
  function waitAll<A1, B1, A2, B2>(ts: [Task<A1, B1>, Task<A2, B2>]): Task<A1 | A2, [B1, B2]>;
