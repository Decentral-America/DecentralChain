...
## About the image
This Docker image contains scripts and configs to run the DecentralChain Pairs Daemon. The image is focused on work with pairs precalculation.

Container downloads source code and configuration files from the [repository](https://github.com/Decentral-America/DecentralChain/tree/main/apps/data-service) and runs it.  
 
## Requirements ⚠️

1. PostgreSQL 10 database with a table structure found in [Decentral-America/blockchain-postgres-sync](https://github.com/Decentral-America/blockchain-postgres-sync)
...
```
docker run -e PGHOST=*** -e PGDATABASE=*** -e PGUSER=*** -e PGPASSWORD=*** -it ghcr.io/decentral-america/data-service:pairs-daemon
```
