...
export class LedgerWallet extends Wallet<WalletPrivateDataOfType<'ledger'>> {
  ...
  override async signAuth(dataBuffer: Uint8Array) {
    const signature = await this.#ledger.signRequest({ dataBuffer });

    return base58Decode(signature);
  }

  override async signCancelOrder(dataBuffer: Uint8Array) {
    const signature = await this.#ledger.signRequest({ dataBuffer });

    return base58Decode(signature);
  }

  override async signOrder(dataBuffer: Uint8Array, dataVersion: 1 | 2 | 3 | 4) {
    const signature = await this.#ledger.signOrder({ dataBuffer, dataVersion });

    return base58Decode(signature);
  }

  override async signRequest(dataBuffer: Uint8Array) {
    const signature = await this.#ledger.signRequest({ dataBuffer });

    return base58Decode(signature);
  }

  override async signTx(dataBuffer: Uint8Array, tx: MessageTx) {
