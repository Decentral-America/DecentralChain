...
### Data source
`Ecosystem/dcc-mass-transfer/real_wallets_2000_details.csv` — 2000 pre-generated wallets with seeds.
Format: `address,seed,public_key` (check actual CSV header — it may differ).

This CSV must be stored securely on Newark at `/opt/dcc/test-wallets.csv` (age-encrypted or permission-restricted). The admin-dashboard server reads it at runtime for balance queries and signing.
...
| `generate_wallets.py` | Create new test wallet sets | ❌ Not critical — existing 2000 wallets sufficient |
| `real_wallets_2000_details.csv` | 2000 pre-funded test wallets with seeds | ⚠️ Verify current migration status — this doc could not confirm from source alone whether the CSV has been copied to `/opt/dcc/test-wallets.csv` on Newark since 2026-06-21; check the server directly before relying on this row |
| `k6/` | Alternative load testing via k6 | ✅ replaced by Rust binary (better) |

**Critical**: `real_wallets_2000_details.csv` must be securely copied to Newark before Treasury can work.
```bash
# Encrypt and copy wallet CSV to Newark
age --encrypt --recipient age1gpe6clrwzvfjlkwm2ww4a00f0ktk49raaslgs2zr3r0vzkcez9fsd97vzs \
    -o /tmp/test-wallets.csv.age \
    Ecosystem/dcc-mass-transfer/real_wallets_2000_details.csv
scp -i deploy_key_testnet /tmp/test-wallets.csv.age deploy@66.228.55.154:/opt/dcc/test-wallets.csv.age
```
...
|---|---|---|
| Wallet CSV not on Newark | HIGH | Encrypt with age + scp (see section 14); set `DCC_WALLET_CSV_PATH=/opt/dcc/test-wallets.csv` (already default) |
