...
function createClearStore<T>(
  addContainer: TStore<T>,
  addRemoveF: IRemoveOrderFunc<Partial<T>>,
  idKey: keyof T,
) {
...
function createClearStoreAll<T>(container: TStore<T>, _addRemoveF: IRemoveOrderFunc<Partial<T>>) {
  return () => {
