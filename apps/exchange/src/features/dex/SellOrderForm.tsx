...
export const SellOrderForm: React.FC = () => {
  ...
  const { amountDecimals } = pairDecimals;
  ...
  // asset's decimals — not a fixed 10^8. Selling spends the amount asset.
  const availableBalance = useMemo(() => {
    const coins = isDccAmountAsset
      ? (dccBalances?.available ?? 0)
      : (assetBalanceData?.balance ?? 0);
    return coinsToTokens(coins, amountDecimals);
  }, [isDccAmountAsset, dccBalances?.available, assetBalanceData?.balance, amountDecimals]);
